...
def plot_total_reward_runs(list_of_exp_dictionaries, optimal_reward_mean, optimal_reward_std, labels, title, window=100):
...
def plot_total_loss_runs(list_of_exp_dictionaries, labels, title, window=100):
    ...
    for i,d in enumerate(list_of_exp_dictionaries):
        ...
        plt.plot(np.arange(len(average_losses)), average_losses, label=labels[i])
    plt.xlabel("Number of optimizer steps", fontsize=16)
